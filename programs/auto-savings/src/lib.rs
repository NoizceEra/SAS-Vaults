...
pub mod auto_savings {
    ...
    // ============================================================================
    // Multi-Account Allocation Instructions
    // ============================================================================

    /// Initialize allocation config for a user
    pub fn initialize_allocation_config(ctx: Context<InitializeAllocationConfig>) -> Result<()> {
        let allocation_config = &mut ctx.accounts.allocation_config;
        allocation_config.owner = ctx.accounts.user.key();
        allocation_config.allocations = Vec::new();
        allocation_config.total_allocations = 0;
        allocation_config.bump = ctx.bumps.allocation_config;

        msg!("Allocation config initialized for user");
        Ok(())
    }
    ...
    pub fn create_allocation(
        ...
        require!(
            ...
            ErrorCode::InvalidAllocationName
        );
        require!(
            percentage > 0 && percentage <= 100,
            ErrorCode::InvalidAllocationPercentage
        );
        ...
        require!(
            allocation_config.total_allocations < 10,
            ErrorCode::TooManyAllocations
        );

        // Calculate total percentage including new allocation
        let current_total: u16 = allocation_config
            .allocations
            .iter()
            .filter(|a| a.is_active)
            .map(|a| a.percentage as u16)
            .sum();

        require!(
            current_total + (percentage as u16) <= 100,
            ErrorCode::AllocationPercentageExceeded
        );

        // Create new allocation
        let index = allocation_config.total_allocations;
        let user_key = ctx.accounts.user.key();
        let (vault_pda, _bump) = Pubkey::find_program_address(
            &[b"allocation_vault", user_key.as_ref(), &[index]],
            ctx.program_id,
        );
        ...
        allocation_config.allocations.push(allocation);
        allocation_config.total_allocations = allocation_config
            .total_allocations
            .checked_add(1)
            .ok_or(ErrorCode::Overflow)?;
        ...
        msg!(
            "Created allocation: {} ({}%)",
            allocation_config.allocations.last().unwrap().name,
            percentage
        );
        Ok(())
    }
    ...
    pub fn update_allocation(
        ...
        // Validate percentage first (before mutable borrow of allocation)
        if let Some(new_percentage) = percentage {
            require!(
                new_percentage > 0 && new_percentage <= 100,
                ErrorCode::InvalidAllocationPercentage
            );

            // Calculate total percentage excluding current allocation
            let other_total: u16 = allocation_config
                .allocations
                .iter()
                .enumerate()
                .filter(|(i, a)| *i != index as usize && a.is_active)
                .map(|(_, a)| a.percentage as u16)
                .sum();

            require!(
                other_total + (new_percentage as u16) <= 100,
                ErrorCode::AllocationPercentageExceeded
            );
        }
        ...
        msg!("Updated allocation at index {}", index);
        Ok(())
    }

    /// Remove an allocation (mark as inactive)
    pub fn remove_allocation(ctx: Context<RemoveAllocation>, index: u8) -> Result<()> {
        let allocation_config = &mut ctx.accounts.allocation_config;
        ...
        msg!("Removed allocation at index {}", index);
        Ok(())
    }
...
/// Multi-account allocation configuration
/// Allows users to split savings across multiple named allocations
#[account]
#[derive(InitSpace)]
pub struct AllocationConfig {
    pub owner: Pubkey, // 32 bytes - user who owns this config
    #[max_len(10)]
    pub allocations: Vec<Allocation>, // Vector of allocations (max 10)
    pub total_allocations: u8, // 1 byte - count of active allocations
    pub bump: u8,      // 1 byte
}

/// Individual allocation within an AllocationConfig
#[derive(AnchorSerialize, AnchorDeserialize, Clone, InitSpace)]
pub struct Allocation {
    #[max_len(32)]
    pub name: String, // Allocation name (e.g., "Emergency Fund")
    pub percentage: u8,       // Percentage of deposits (0-100)
    pub vault: Pubkey,        // 32 bytes - PDA for this allocation's vault
    pub total_saved: u64,     // 8 bytes - total saved in this allocation
    pub total_withdrawn: u64, // 8 bytes - total withdrawn
    pub is_active: bool,      // 1 byte - can be deactivated
}
...
pub enum ErrorCode {
    ...
    #[msg("Allocation name must be 1-32 characters")]
    InvalidAllocationName,

    #[msg("Allocation percentage must be 1-100")]
    InvalidAllocationPercentage,

    #[msg("Total allocation percentages cannot exceed 100")]
    AllocationPercentageExceeded,
