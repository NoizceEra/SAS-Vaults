# PowerShell script to replace allocation functions in lib.rs

$libPath = "programs\auto-savings\src\lib.rs"
$content = Get-Content $libPath -Raw

# Define the new deposit_with_allocation function
$newDepositFunction = @'
...
# Define the new withdraw_from_allocation function
$newWithdrawFunction = @'
...
# Use regex to replace the deposit function
$depositPattern = '(?s)    /// Deposit SOL and split across allocations based on percentages.*?    \}\r?\n\r?\n    /// Withdraw from a specific allocation'
$content = $content -replace $depositPattern, ($newDepositFunction + "`r`n`r`n    /// Withdraw from a specific allocation")

# Use regex to replace the withdraw function  
$withdrawPattern = '(?s)    /// Withdraw from a specific allocation.*?    \}\r?\n\}'
$content = $content -replace $withdrawPattern, ($newWithdrawFunction + "`r`n}")

# Write back to file
$content | Set-Content $libPath -NoNewline

Write-Host "✅ Functions replaced successfully!" -ForegroundColor Green
